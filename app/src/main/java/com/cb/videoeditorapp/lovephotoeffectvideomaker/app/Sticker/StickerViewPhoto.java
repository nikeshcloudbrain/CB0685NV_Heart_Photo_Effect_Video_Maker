 ...
 * @author wupanjie
 */
public class StickerViewPhoto extends FrameLayout {
  ...
  private final List<StickerPhoto> stickers = new ArrayList<>();
  private final List<BitmapStickerIconPhoto> icons = new ArrayList<>(4);
  ...
  private BitmapStickerIconPhoto currentIcon;
  ...
  private StickerPhoto handlingSticker;
  ...
  public StickerViewPhoto(Context context) {
    this(context, null);
  }

  public StickerViewPhoto(Context context, AttributeSet attrs) {
    this(context, attrs, 0);
  }

  public StickerViewPhoto(Context context, AttributeSet attrs, int defStyleAttr) {
    super(context, attrs, defStyleAttr);
    touchSlop = ViewConfiguration.get(context).getScaledTouchSlop();
    TypedArray a = null;
    try {
      a = context.obtainStyledAttributes(attrs, R.styleable.StickerViewLove);
      showIcons = a.getBoolean(R.styleable.StickerViewLove_showIcons, false);
      showBorder = a.getBoolean(R.styleable.StickerViewLove_showBorder, false);
      bringToFrontCurrentSticker =
          a.getBoolean(R.styleable.StickerViewLove_bringToFrontCurrentSticker, false);

      borderPaint.setAntiAlias(true);
      borderPaint.setColor(a.getColor(R.styleable.StickerViewLove_borderColor, getResources().getColor(R.color.colorPrimary)));
      borderPaint.setAlpha(a.getInteger(R.styleable.StickerViewLove_borderAlpha, 128));
  ...
  public void configDefaultIcons() {
    BitmapStickerIconPhoto deleteIcon = new BitmapStickerIconPhoto(
        ContextCompat.getDrawable(getContext(), R.drawable.sticker_ic_close_photo),
        BitmapStickerIconPhoto.LEFT_TOP);

    deleteIcon.setIconEvent(new DeleteIconEventPhoto());
    BitmapStickerIconPhoto zoomIcon = new BitmapStickerIconPhoto(
        ContextCompat.getDrawable(getContext(), R.drawable.sticker_ic_scale_photo),
        BitmapStickerIconPhoto.RIGHT_BOTOM);
    zoomIcon.setIconEvent(new ZoomIconEventPhoto());
    BitmapStickerIconPhoto flipIcon = new BitmapStickerIconPhoto(
        ContextCompat.getDrawable(getContext(), R.drawable.sticker_ic_flip_photo),
  ...
  public void sendToLayer(int oldPos, int newPos) {
    if (stickers.size() >= oldPos && stickers.size() >= newPos) {
      StickerPhoto s = stickers.get(oldPos);
  ...
  protected void drawStickers(Canvas canvas) {
    for (int i = 0; i < stickers.size(); i++) {
      StickerPhoto sticker = stickers.get(i);
      if (sticker != null) {
        sticker.draw(canvas);
      }
    }
    ...
    if (handlingSticker != null && !locked && (showBorder || showIcons)) {
      ...
      if (showIcons) {
        ...
        for (int i = 0; i < icons.size(); i++) {
          BitmapStickerIconPhoto icon = icons.get(i);
          switch (icon.getPosition()) {
            case BitmapStickerIconPhoto.LEFT_TOP:

              configIconMatrix(icon, x1, y1, rotation);
              break;

            case BitmapStickerIconPhoto.RIGHT_TOP:
              configIconMatrix(icon, x2, y2, rotation);
              break;

            case BitmapStickerIconPhoto.LEFT_BOTTOM:
              configIconMatrix(icon, x3, y3, rotation);
              break;

            case BitmapStickerIconPhoto.RIGHT_BOTOM:
  ...
  protected void configIconMatrix(@NonNull BitmapStickerIconPhoto icon, float x, float y,
                                  float rotation) {
    icon.setX(x);
    icon.setY(y);
  ...
  public void zoomAndRotateSticker(@Nullable StickerPhoto sticker, @NonNull MotionEvent event) {
    if (sticker != null) {
  ...
  protected void constrainSticker(@NonNull StickerPhoto sticker) {
    float moveX = 0;
    float moveY = 0;
    int width = getWidth();
  ...
  @Nullable
  protected BitmapStickerIconPhoto findCurrentIconTouched() {
    for (BitmapStickerIconPhoto icon : icons) {
   ...
   **/
  @Nullable
  protected StickerPhoto findHandlingSticker() {
  ...
  protected boolean isInStickerArea(@NonNull StickerPhoto sticker, float downX, float downY) {
    tmp[0] = downX;
    tmp[1] = downY;
   ...
   **/
  protected void transformSticker(@Nullable StickerPhoto sticker) {
    if (sticker == null) {
      FunctionsPhoto.LogE(TAG, "transformSticker: the bitmapSticker is null or the bitmapSticker bitmap is null");
      return;
    }
  ...
  public void flip(@Nullable StickerPhoto sticker, @Flip int direction) {
    if (sticker != null) {
  ...
  public boolean replace(@Nullable StickerPhoto sticker) {
    return replace(sticker, true);
  }

  public boolean replace(@Nullable StickerPhoto sticker, boolean needStayState) {
  ...
  public boolean remove(@Nullable StickerPhoto sticker) {
    if (stickers.contains(sticker)) {
      ...
      FunctionsPhoto.LogD(TAG, "remove: the sticker is not in this StickerView");
  ...
  @NonNull
  public StickerViewPhoto addSticker(@NonNull StickerPhoto sticker) {
    return addSticker(sticker, StickerPhoto.Position.CENTER);
  }

  public StickerViewPhoto addSticker(@NonNull final StickerPhoto sticker,
                                     final @StickerPhoto.Position int position) {
  ...
  protected void addStickerImmediately(@NonNull StickerPhoto sticker, @StickerPhoto.Position int position) {
  ...
  protected void setStickerPosition(@NonNull StickerPhoto sticker, @StickerPhoto.Position int position) {
    float width = getWidth();
    float height = getHeight();
    float offsetX = width - sticker.getWidth();
    float offsetY = height - sticker.getHeight();
    if ((position & StickerPhoto.Position.TOP) > 0) {
      offsetY /= 4f;
    } else if ((position & StickerPhoto.Position.BOTTOM) > 0) {
      offsetY *= 3f / 4f;
    } else {
      offsetY /= 2f;
    }
    if ((position & StickerPhoto.Position.LEFT) > 0) {
      offsetX /= 4f;
    } else if ((position & StickerPhoto.Position.RIGHT) > 0) {
      offsetX *= 3f / 4f;
    } else {
      offsetX /= 2f;
    }
  ...
  @NonNull
  public float[] getStickerPoints(@Nullable StickerPhoto sticker) {
    float[] points = new float[8];
    getStickerPoints(sticker, points);
    return points;
  }

  public void getStickerPoints(@Nullable StickerPhoto sticker, @NonNull float[] dst) {
    if (sticker == null) {
      Arrays.fill(dst, 0);
      return;
    }
  ...
  public void save(@NonNull File file) {
    try {
      StickerUtilsPhoto.saveImageToGallery(file, createBitmap());
      StickerUtilsPhoto.notifySystemGallery(getContext(), file);
  ...
  @NonNull
  public StickerViewPhoto setLocked(boolean locked) {
    this.locked = locked;
    invalidate();
    return this;
  }
  ...
  @NonNull
  public StickerViewPhoto setMinClickDelayTime(int minClickDelayTime) {
  ...
  @NonNull
  public StickerViewPhoto setConstrained(boolean constrained) {
  ...
  @NonNull
  public StickerViewPhoto setOnStickerOperationListener(
  ...
  @Nullable
  public StickerPhoto getCurrentSticker() {
    return handlingSticker;
  }

  @NonNull
  public List<BitmapStickerIconPhoto> getIcons() {
    return icons;
  }

  public void setIcons(@NonNull List<BitmapStickerIconPhoto> icons) {
    this.icons.clear();
  ...
  public interface OnStickerOperationListener {
    void onStickerAdded(@NonNull StickerPhoto sticker);

    void onStickerClicked(@NonNull StickerPhoto sticker);

    void onStickerDeleted(@NonNull StickerPhoto sticker);

    void onStickerDragFinished(@NonNull StickerPhoto sticker);

    void onStickerTouchedDown(@NonNull StickerPhoto sticker);

    void onStickerZoomFinished(@NonNull StickerPhoto sticker);

    void onStickerFlipped(@NonNull StickerPhoto sticker);

    void onStickerDoubleTapped(@NonNull StickerPhoto sticker);
  }
}
